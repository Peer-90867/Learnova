  ...
  const generatePresentation = async () => {
    ...
    try {
      ...
      if (cached) {
        ...
        setLoading(false);
        showToast('Presentation loaded from cache!', 'success');
        return;
      }
      ...
      const response = await ai.models.generateContent({
        ...
        The language of the presentation should be ${user.settings?.language || 'English'} and the tone should be ${user.settings?.tone || 'Academic'}.
      ...
      if (error?.message?.includes('429') || error?.status === 429 || error?.message?.includes('exceeded your current quota')) {
        showToast('You have exceeded your Gemini API quota.', 'error');
      } else {
